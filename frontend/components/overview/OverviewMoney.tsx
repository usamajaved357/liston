...
import { MoneySummary } from "@/lib/api";
...
export type Metric = "sales" | "fees" | "earnings" | "sourceCost" | "profit";
...
const CARDS: Record<Metric, Card[]> = {
  sales: [
...
export function MetricTabs({ metric, onMetric }: { metric: Metric; onMetric: (m: Metric) => void }) {
  return (
    <div role="tablist" aria-label="Figure" className="flex gap-1 overflow-x-auto border-b border-[var(--color-line)]">
...
export function MetricCards({ metric, summaries, loading }: { metric: Metric; summaries: MoneySummary[]; loading?: boolean }) {
          ...
          <div key={card.label} className="card flex min-h-[136px] flex-col p-5">
            ...
            {loading || !main ? (
              ...
              <>
                ...
                {figure.kind === "money" &&
                  // A market eBay has no figures for yet isn't a zero: left out.
                  others.filter((o) => metric === "sales" || o.withEarnings > 0).map((o) => (
